...
namespace NetDuino.Hardware.HT1632
{
  ...
  */
  ...
  public class MatrixDisplay32x8 : IMatrixDisplay
  {
    private const int BACKBUFFER_SIZE = 32;
...
//        throw new ArgumentOutOfRangeException( "displayNum" );
                          ...
                          bool useShadow )
    {
      ...
      address = ( _backBufferSize * displayNum ) + address;
    ...
    private static byte DisplayXYToIndex( byte x,
                                          byte y )
    {
      int addresss = x << 1; // Calculate which quandrant[?] it's in 
      addresss += y >= 4 ? 1 : 0;
      return (byte) addresss;
    }

    // Converts caretesian coordinate to the custom display buffer index
    private byte xyToIndex( byte x,
                            byte y )
    {
      // cap X coordinate at 32 column
      x &= 0x1F;
